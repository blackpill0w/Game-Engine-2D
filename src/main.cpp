...
int main() {
  ...
  engine.input_manager.bind_key_while_pressed(e2d::KeyEvent::Key::Right, [&] {
    engine.animation_manager.continue_animation(laura_id);
    engine.animation_manager.set_animation_state(
        laura_id, size_t(LauraAnimationStates::FacingRight)
    );
    engine.world.get_character(laura_id)->sprite.move(speed, 0.f);
  });
  engine.input_manager.bind_key_while_pressed(e2d::KeyEvent::Key::Left, [&] {
    engine.animation_manager.continue_animation(laura_id);
    engine.animation_manager.set_animation_state(
        laura_id, size_t(LauraAnimationStates::FacingLeft)
    );
    engine.world.get_character(laura_id)->sprite.move(-speed, 0.f);
  });

  engine.input_manager.bind_key_while_pressed(e2d::KeyEvent::Key::Up, [&] {
    engine.animation_manager.continue_animation(laura_id);
    engine.animation_manager.set_animation_state(laura_id, size_t(LauraAnimationStates::FacingUp));
    engine.world.get_character(laura_id)->sprite.move(0.f, -speed);
  });

  engine.input_manager.bind_key_while_pressed(e2d::KeyEvent::Key::Down, [&] {
    engine.animation_manager.continue_animation(laura_id);
    engine.animation_manager.set_animation_state(
        laura_id, size_t(LauraAnimationStates::FacingDown)
    );
    engine.world.get_character(laura_id)->sprite.move(0.f, speed);
  });

  engine.input_manager.bind(
      std::make_unique<e2d::KeyEvent>(e2d::EventType::KeyPress, e2d::KeyEvent::Key::F),
      [&] { fmt::println("F"); }
  );
